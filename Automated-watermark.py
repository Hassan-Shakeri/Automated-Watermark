...
class WatermarkAutomation:
    ...
    # list the photos in the pre_photos file and check their extensions
    def run(self):
        pictures = self.get_file_from_in_directory()
        self.put_watermark_on_multiple_files(pictures)
        #self.delet_image_from_directory(pictures)

    def delet_image_from_directory(self, images):
        for image in images:
            os.remove(f'{self.in_directory}/{image}')

    def get_file_from_in_directory(self):
    ...
    def put_watermark_on_single_file(self, file_name):
        ...
        image.paste(self.watermark, (int((image_width - self.watermark_width) / 2),
                                     int((image_height - self.watermark_height) / 2)), self.watermark)
